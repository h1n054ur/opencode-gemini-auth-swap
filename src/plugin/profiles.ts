...
export function setRefreshToken(refreshToken: string): boolean {
  try {
    ...
    // Update or add account at active index
    const activeIndex = data.activeIndex ?? 0;
    if (data.accounts[activeIndex]) {
      data.accounts[activeIndex].refreshToken = refreshToken;
      data.accounts[activeIndex].lastUsed = Date.now();
    } else {
      data.accounts[activeIndex] = {
        refreshToken,
        addedAt: Date.now(),
        lastUsed: Date.now(),
        rateLimitResetTimes: {},
